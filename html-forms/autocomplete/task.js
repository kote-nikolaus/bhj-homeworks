class Autocomplete {
  ...
  getMatches( text ) {
    let optionsArray = Array.from(this.input.options);
    let result = [];
    for (let i = 0; i < optionsArray.length; i++) {
      if (optionsArray[i].text.toLowerCase().includes(text.toLowerCase())) {
        result.push({text: optionsArray[i].text, value: optionsArray[i].value});
      }
    }
    return result;
  }
